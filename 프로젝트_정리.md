...
- 이상 탐지 시스템으로 공정 이상 조기 발견
- [정량적 성과: 예측 정확도 R² 0.XX 달성, 이상 탐지 정확도 XX% 등 - 추후 추가 가능]
...
- ML 라이브러리: PyTorch, Tensorflow, scikit-learn
...
- 임베딩: OpenAI Embeddings, Upstage, LlamaParse
- 문서 파싱:  Docling, Unstructured, pypdf, docx2txt, python-pptx, pdf2image
- 버전 관리: Git, GitHub
- 패키지 관리: Uv, Bun
...
### 프로젝트 관리
- 1인 개발에서 3명 팀 PL로 성장 (한국자동차연구원 프로젝트)
- 요구사항 분석, 일정 관리, 팀원 코드 리뷰
- 200+ 테스트 케이스 작성으로 체계적인 품질 관리

### 아키텍처 설계
- LangGraph 멀티 에이전트 시스템 설계
- RESTful API 설계 및 문서화
- Docker 기반 마이크로서비스 아키텍처

### 문제 해결
- 복잡한 AI 워크플로우 구현 (Research → Report → Supervisor Agent)
- 하이브리드 검색 (Dense + Sparse) 도입으로 검색 정확도 향상
- 실시간 스트리밍 응답 시스템 구축

### 오픈소스 기여
- LangChain Open Tutorial 프로젝트 기여
...
### AI/ML
- LLM 통합: OpenAI API (GPT-3.5/4/4o), Claude, Private Model
- LLM 프레임워크: LangChain, LangGraph (멀티 에이전트 시스템)
- RAG (Retrieval-Augmented Generation): 하이브리드 검색 (Dense + Sparse), Multi-Query 생성
- ML 라이브러리: PyTorch, LightGBM, XGBoost, CatBoost, scikit-learn
- 이상 탐지: PyOD, COMBO
- Computer Vision: [이미지 분류 모델 개발 경험]

### Backend
- 언어: Python
- 프레임워크: FastAPI, Flask
- 비동기 처리: Celery, Redis
- ORM: Kysely (TypeScript), SQLAlchemy (추정)
- 시계열 분석: statsmodels, NumPy, Pandas
...
- UI 라이브러리: [추정: Tailwind CSS, Shadcn UI 등]

### Database
- 관계형 DB: PostgreSQL, MySQL
- 벡터 DB: Qdrant (하이브리드 검색, 컬렉션 관리)
- 캐시: Redis (캐싱 레이어, TTL, LRU)

### DevOps & Infrastructure
- 컨테이너: Docker, Docker Compose
- 웹서버: Nginx, Gunicorn
- 인증: Keycloak (SSO, OAuth 2.0)
- 모니터링: LangSmith
