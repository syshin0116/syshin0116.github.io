...
draft: false
enableToc: true
description: 멀티에이전트 시스템 아키텍처 패턴과 대규모 구현 전략을 다룬다. LangGraph Handoffs·Command·Subgraphs, 계층적·네트워크·중재자·발행구독 패턴, Supervisor Tool-Calling, MCP·A2A·ANP·FIPA 프로토콜, 샤딩·로드밸런싱·내결함성을 설명한다.
summary: 멀티에이전트 시스템은 단일 에이전트의 순차 처리·컨텍스트 제약·단일 장애점 한계를 극복하여 분산 처리·전문화·확장성·내결함성을 제공한다. LangGraph는 Command 객체로 Handoffs 핸드오프 제어권 전달을 구현하고 Subgraphs로 재사용성·모듈화를 지원한다. 계층적 아키텍처는 Master-Supervisor-Worker 구조로 중앙 집중식 의사결정하고, 네트워크(P2P) 아키텍처는 수평적 직접 통신으로 높은 내결함성을 제공하며, 중재자 아키텍처는 Supervisor 패턴으로 통신 복잡성을 감소시키고, 발행-구독 아키텍처는 Message Broker 기반 이벤트 비동기 통신으로 느슨한 결합을 구현한다. MCP는 AI 모델-데이터 연결, A2A는 에이전트 간 상호 운용성, ANP는 에이전트 네트워킹, FIPA는 표준 통신 프로토콜을 제공한다. 대규모 환경에서는 샤딩·로드밸런싱·내결함성 메커니즘으로 확장성을 확보한다.
published: *id001
modified: *id001
categories:
- AI
...
> [!summary]
>
> 멀티에이전트 시스템은 단일 에이전트의 순차 처리·컨텍스트 제약·단일 장애점 한계를 극복하여 분산 처리·전문화·확장성·내결함성을 제공한다. LangGraph는 Command 객체로 Handoffs 핸드오프 제어권 전달을 구현하고 Subgraphs로 재사용성·모듈화를 지원한다. 계층적 아키텍처는 Master-Supervisor-Worker 구조로 중앙 집중식 의사결정하고, 네트워크(P2P) 아키텍처는 수평적 직접 통신으로 높은 내결함성을 제공하며, 중재자 아키텍처는 Supervisor 패턴으로 통신 복잡성을 감소시키고, 발행-구독 아키텍처는 Message Broker 기반 이벤트 비동기 통신으로 느슨한 결합을 구현한다. MCP는 AI 모델-데이터 연결, A2A는 에이전트 간 상호 운용성, ANP는 에이전트 네트워킹, FIPA는 표준 통신 프로토콜을 제공한다. 대규모 환경에서는 샤딩·로드밸런싱·내결함성 메커니즘으로 확장성을 확보한다.
