...
draft: false
enableToc: true
description: TAM 비디오 객체 추적·분할 모델을 다룬다. SAM+XMem 통합, VOT·VOS, 4단계 프로세스(SAM 초기화·XMem 추적·SAM 개선·사용자 수정), ViT 기반 제로샷 분할, 시간적 대응, 단일 패스 추론, 샷 변경 처리, 비디오 인페인팅을 설명한다.
summary: Track Anything Model(TAM)은 SAM의 ViT 기반 이미지 분할과 XMem의 반지도 VOS를 결합한 대화형 비디오 추적·분할 모델이다. SAM으로 클릭 기반 초기화하여 관심 객체 마스크를 생성하고, XMem으로 시간적·공간적 대응성 기반 다음 프레임 예측하며, SAM으로 마스크 개선하고 사용자 수정으로 어려운 시나리오를 처리한다. 단일 패스 추론으로 다중 객체 분리·대상 변형·스케일 변경·카메라 움직임을 처리하고, 긴 비디오에서 샷 변경을 지원하며, 비디오 주석·장기 객체 추적·비디오 편집·시각화 개발 툴킷에 활용된다. 제로샷 분할로 추가 학습 없이 실시간 마스크를 생성하지만, 복잡한 객체 구조와 장기 추적 실패 복구에 한계가 있다.
...
> [!summary]
>
> Track Anything Model(TAM)은 SAM의 ViT 기반 이미지 분할과 XMem의 반지도 VOS를 결합한 대화형 비디오 추적·분할 모델이다. SAM으로 클릭 기반 초기화하여 관심 객체 마스크를 생성하고, XMem으로 시간적·공간적 대응성 기반 다음 프레임 예측하며, SAM으로 마스크 개선하고 사용자 수정으로 어려운 시나리오를 처리한다. 단일 패스 추론으로 다중 객체 분리·대상 변형·스케일 변경·카메라 움직임을 처리하고, 긴 비디오에서 샷 변경을 지원하며, 비디오 주석·장기 객체 추적·비디오 편집·시각화 개발 툴킷에 활용된다. 제로샷 분할로 추가 학습 없이 실시간 마스크를 생성하지만, 복잡한 객체 구조와 장기 추적 실패 복구에 한계가 있다.
