...
draft: false
enableToc: true
description: 비선형 차원 축소 기법 t-SNE의 원리와 활용을 다룬다. t-distributed stochastic neighbor embedding, 고차원→2D/3D 시각화, PCA 대비 비선형 관계 보존, 조건부 확률 분포, KL-divergence 최소화, Student t-분포, perplexity 5-50, O(n²) 복잡도를 설명한다.
summary: t-SNE는 고차원 데이터를 2D/3D로 축소하는 비선형 차원 축소 기법으로 PCA 대비 비선형 관계를 잘 보존한다. 고차원에서 유사한 점은 저차원에서도 가깝게, 다른 점은 멀리 배치하며 국소적 구조를 우선 보존한다. 알고리즘은 고차원에서 조건부 확률(정규 분포)을 계산하고 저차원에서도 확률(Student t-분포 df=1)을 계산하여, 두 확률 분포 간 KL-divergence를 경사 하강법으로 최소화한다. perplexity 5-50은 효과적 이웃 수를 결정하고, iterations 250-1000으로 최적화하며, O(n²) 계산 복잡도가 단점이다. MNIST 손글씨, 얼굴 인식, 단어 임베딩, 소셜 네트워크 시각화에 활용되며 군집 구조를 시각적으로 잘 분리하지만 전역 구조는 보존하지 못한다.
...
> [!summary]
>
> t-SNE는 고차원 데이터를 2D/3D로 축소하는 비선형 차원 축소 기법으로 PCA 대비 비선형 관계를 잘 보존한다. 고차원에서 유사한 점은 저차원에서도 가깝게, 다른 점은 멀리 배치하며 국소적 구조를 우선 보존한다. 알고리즘은 고차원에서 조건부 확률(정규 분포)을 계산하고 저차원에서도 확률(Student t-분포 df=1)을 계산하여, 두 확률 분포 간 KL-divergence를 경사 하강법으로 최소화한다. perplexity 5-50은 효과적 이웃 수를 결정하고, iterations 250-1000으로 최적화하며, O(n²) 계산 복잡도가 단점이다. MNIST 손글씨, 얼굴 인식, 단어 임베딩, 소셜 네트워크 시각화에 활용되며 군집 구조를 시각적으로 잘 분리하지만 전역 구조는 보존하지 못한다.
