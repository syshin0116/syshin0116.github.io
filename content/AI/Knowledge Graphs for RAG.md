...
draft: false
enableToc: true
description: RAG 시스템에서 지식 그래프 활용과 Neo4j 구현을 다룬다. 노드(entities)·관계(relationships) 구조, Cypher 쿼리 언어, MATCH 패턴 매칭, 속성 그래프, 라벨·타입·방향, 구조화된 정보 검색, 컨텍스트 강화, 추론 지원을 설명한다.
summary: 지식 그래프는 노드와 관계로 정보를 저장하는 데이터베이스로 RAG 시스템의 정보 검색과 컨텍스트를 향상시킨다. 노드는 개체(entity)를 표현하고 라벨로 그룹화되며, 관계는 노드 간 연결로 항상 타입과 방향을 가진다. 노드와 엣지 모두 키/값 구조의 속성을 가질 수 있어 풍부한 정보 표현이 가능하다. Neo4j는 그래프 데이터베이스로 Cypher 쿼리 언어를 사용하며, MATCH 패턴 매칭으로 복잡한 관계를 검색한다. 구조화된 정보 검색으로 키워드 검색을 넘어 관계 기반 검색이 가능하고, 관련 개체와 관계 정보로 LLM에 풍부한 컨텍스트를 제공하며, 그래프 구조로 복잡한 추론을 지원한다.
published: 2024-08-15
modified: 2024-08-15
---
> [!summary]
>
> 지식 그래프는 노드와 관계로 정보를 저장하는 데이터베이스로 RAG 시스템의 정보 검색과 컨텍스트를 향상시킨다. 노드는 개체(entity)를 표현하고 라벨로 그룹화되며, 관계는 노드 간 연결로 항상 타입과 방향을 가진다. 노드와 엣지 모두 키/값 구조의 속성을 가질 수 있어 풍부한 정보 표현이 가능하다. Neo4j는 그래프 데이터베이스로 Cypher 쿼리 언어를 사용하며, MATCH 패턴 매칭으로 복잡한 관계를 검색한다. 구조화된 정보 검색으로 키워드 검색을 넘어 관계 기반 검색이 가능하고, 관련 개체와 관계 정보로 LLM에 풍부한 컨텍스트를 제공하며, 그래프 구조로 복잡한 추론을 지원한다.
