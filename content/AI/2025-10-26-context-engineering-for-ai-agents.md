...
draft: false
enableToc: true
description: AI 에이전트의 컨텍스트 폭증 문제를 해결하는 5가지 실전 기법을 다룬다. 오프로딩(파일 시스템), 축소(압축/요약), 검색(RAG), 격리(통신/메모리), 캐싱(프롬프트 캐싱), 계층형 액션 공간, MCP 통합, 파인튜닝 대비 장점을 LangChain과 Manus 웨비나 내용으로 정리한다.
summary: AI 에이전트는 도구 호출마다 컨텍스트가 증가하여 200K 토큰 전후 Context Rot이 발생한다. 해결책은 오프로딩(도구 출력을 파일로 저장), 축소(압축은 정확성, 요약은 가독성), 검색(RAG로 필요시 로드), 격리(통신은 독립성, 메모리 공유는 효율성), 캐싱(반복 컨텍스트 재사용)이다. 계층형 액션 공간은 정책 네트워크로 고수준 도구만 노출하고, MCP는 무한 확장 가능한 에이전트 설계를 가능케 한다. 파인튜닝 대비 즉시 반영과 생태계 변화 대응이 강점이다.
...
> [!summary]
>
> AI 에이전트는 도구 호출마다 컨텍스트가 증가하여 200K 토큰 전후 Context Rot이 발생한다. 해결책은 오프로딩(도구 출력을 파일로 저장), 축소(압축은 정확성, 요약은 가독성), 검색(RAG로 필요시 로드), 격리(통신은 독립성, 메모리 공유는 효율성), 캐싱(반복 컨텍스트 재사용)이다. 계층형 액션 공간은 정책 네트워크로 고수준 도구만 노출하고, MCP는 무한 확장 가능한 에이전트 설계를 가능케 한다. 파인튜닝 대비 즉시 반영과 생태계 변화 대응이 강점이다.
