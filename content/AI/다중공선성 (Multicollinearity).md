...
draft: false
enableToc: true
description: 회귀 분석과 ML의 다중공선성 문제를 다룬다. VIF 5-10 임계값, 상관행렬·조건수·Tolerance 탐지, Ridge/Lasso/Elastic Net 정규화, PCA/PLS 차원 축소, 변수 중심화, RTIS 공정 데이터 처리, 도메인 지식 기반 센서 선택을 설명한다.
summary: 다중공선성은 독립 변수 간 높은 상관관계로 계수 추정 불안정성과 표준 오차 증가를 유발한다. VIF(Variance Inflation Factor)로 진단하며 VIF >10이면 심각하고, Tolerance <0.1이면 문제다. 조건수 ≥30은 심각한 수준이다. 해결책은 변수 제거, Ridge/Lasso/Elastic Net 정규화, PCA/PLS 차원 축소, 변수 중심화다. Ridge는 L2로 모든 계수를 축소하고, Lasso는 L1로 불필요 변수를 0으로 만들며, Elastic Net은 둘을 결합한다. RTIS 공정 데이터는 센서 간 물리적 연관으로 높은 다중공선성이 발생하며, 도메인 지식 기반 그룹화와 클러스터링 대표 센서 선택으로 처리한다.
published: *id001
modified: *id001
---
...
> [!summary]
>
> 다중공선성은 독립 변수 간 높은 상관관계로 계수 추정 불안정성과 표준 오차 증가를 유발한다. VIF(Variance Inflation Factor)로 진단하며 VIF >10이면 심각하고, Tolerance <0.1이면 문제다. 조건수 ≥30은 심각한 수준이다. 해결책은 변수 제거, Ridge/Lasso/Elastic Net 정규화, PCA/PLS 차원 축소, 변수 중심화다. Ridge는 L2로 모든 계수를 축소하고, Lasso는 L1로 불필요 변수를 0으로 만들며, Elastic Net은 둘을 결합한다. RTIS 공정 데이터는 센서 간 물리적 연관으로 높은 다중공선성이 발생하며, 도메인 지식 기반 그룹화와 클러스터링 대표 센서 선택으로 처리한다.
