...
draft: false
enableToc: true
summary: Information Gain은 어떤 속성을 기준으로 데이터를 분할했을 때 얻을 수 있는 엔트로피의 감소량이다. 결정 트리 알고리즘에서는
  Information Gain이 최대가 되는 속성으로 데이터를 분할한다. \text{Information Gain} = \text{Entropy(parent)}
  - \sum \left( \frac{\text{number of samples in child}}{\text{number of samples in
  parent}} \times \text{Entropy(child)} \right)
published: 2023-08-28 09:09 +0900
modified: 2023-08-28 09:09 +0900
layout: post
categories:
- SeSAC
- 혼공 머신러닝+딥러닝
