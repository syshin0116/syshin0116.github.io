...
summary: '새 맥북을 설정할 때 필요한 모든 것들을 정리했다. 주요 내용은: - 필수 유틸리티와 개발 도구들 - 시스템 설정 최적화 방법 -
  개발 환경 구성하기 - 생산성 향상을 위한 추천 앱 - 한 번에 설치하는 방법'
...
> [!summary]
> 
> 새 맥북을 설정할 때 필요한 모든 것들을 정리했다. 주요 내용은: - 필수 유틸리티와 개발 도구들 - 시스템 설정 최적화 방법 - 개발 환경 구성하기 - 생산성 향상을 위한 추천 앱 - 한 번에 설치하는 방법
...
- **LinearMouse**: 마우스 스크롤 방향/속도, 포인터 가속도를 기기별로 세밀하게 조절하는 도구
...
- **Visual Studio Code** / **Cursor**: 코드 에디터
- **Git**: 버전 관리 시스템
- **uv**: Python 버전 및 패키지 관리자 (pyenv + pip 대체)
- **fnm**: Node.js 버전 관리자 (nvm 대체, 빠름)
- **Bun**: JavaScript 런타임 + 패키지 매니저
- **Docker Desktop**: 컨테이너 환경
...
brew install --cask raycast rectangle linearmouse iterm2 visual-studio-code obsidian stats appcleaner keka iina
brew install git python nfd2nfc
```
...
**LinearMouse**는 macOS에서 마우스 스크롤 방향/속도, 포인터 가속도를 기기별 프로필로 세밀하게 조절할 수 있는 오픈소스 도구다. 트랙패드와 외장 마우스를 독립적으로 설정할 수 있다.
...
```
생성된 공개키(`~/.ssh/id_ed25519.pub`)를 GitHub → Settings → SSH and GPG keys에 등록.
...
#### 개인/회사 계정 분리 (디렉토리 기반)
디렉토리별로 Git 이름/이메일을 자동으로 다르게 적용하려면 `includeIf`를 사용한다.
...
이렇게 하면 `~/Documents/github/personal/` 하위 레포에서는 자동으로 개인 계정으로 커밋된다.

### Python (uv)

**uv**는 Python 버전 관리부터 가상환경, 패키지 설치까지 통합 관리하는 도구다. pyenv + pip + venv를 모두 대체한다.

```sh
# 설치
curl -LsSf https://astral.sh/uv/install.sh | sh

# Python 버전 설치 및 글로벌 기본값 설정
uv python install 3.13
uv python install 3.11
uv python pin 3.13 --global

# ~/.zshrc에 추가 (python 명령어 연결)
export PATH="$HOME/.local/share/uv/python/cpython-3.13-macos-aarch64-none/bin:$PATH"
alias python="python3.13"
alias python3="python3.13"
```

프로젝트별로는 `uv init`으로 시작하면 uv가 가상환경과 버전을 자동 관리한다.

### Node.js (fnm)

**fnm**은 Node.js 버전 관리자로, nvm보다 빠르다.
...
- **zsh-autosuggestions**: 이전 명령어를 흐리게 미리 보여줌, `→`로 자동완성
- **zsh-syntax-highlighting**: 명령어 맞으면 초록색, 틀리면 빨간색
...
#### 한글 파일명
- **nfd2nfc**: macOS에서 한글 파일명이 자음/모음 분리되어 깨지는 문제 방지. 백그라운드 데몬으로 자동 실행
  ```sh
