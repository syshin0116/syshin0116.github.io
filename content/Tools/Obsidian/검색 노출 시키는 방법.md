...
현재 `quartz.config.ts`에 Google Analytics가 설정되어 있으므로 이 방법도 가능하다:
```typescript
analytics: {
  provider: "google",
  tagId: "G-XZB0EYZF1G",
...
> [!note] 현재 상황
> 프로젝트에 `google956ddb3a240f8aaa.html` 파일이 이미 있어서 `public/` 폴더로 이동했다. 사이트 빌드 후 배포하면 Google Search Console에서 확인할 수 있다.
