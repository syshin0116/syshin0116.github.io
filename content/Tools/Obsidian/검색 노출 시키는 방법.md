...
> [!warning] HTML 파일 방법이 실패할 경우
> 
> **문제**: "Could not find your site" 오류가 발생하는 경우
> 
> **원인**: Quartz에서 `public/` 폴더는 빌드 시 덮어씌워진다
> 
> **해결방법**:
> 1. **Google Analytics 방법 사용** (가장 쉬움)
> 2. 또는 HTML 파일을 `quartz/static/` 폴더에 넣고 다시 배포

## 2단계: 기본 SEO 설정

### robots.txt 설정

사이트 루트에 `robots.txt` 파일을 생성한다:
